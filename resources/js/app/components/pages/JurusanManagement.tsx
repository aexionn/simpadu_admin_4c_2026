...
export default function JurusanManagement() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      setError("Gagal memuat data jurusan. Periksa koneksi dan coba lagi.");
    } finally {
      setIsLoading(false);
    }
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      let res: Response;
      if (editTarget) {
        res = await api.patch(`/data-master/jurusan/${editTarget.id_jurusan}`, payload);
      } else {
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      const res = await api.delete(`/data-master/jurusan/${deleteTarget.id_jurusan}`);
      if (!res.ok) {
        const json = await res.json().catch(() => ({}));
  ...
  return (
    <div className="p-8">
      {/* Header */}
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900">Manajemen Jurusan</h1>
        <p className="text-gray-600 mt-1">Kelola data jurusan di POLIBAN</p>
      </div>
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        {isLoading ? (
          ...
          <div className="text-center py-20 text-gray-400">
            {searchTerm ? "Tidak ada hasil yang cocok." : "Belum ada data jurusan."}
          </div>
        ) : (
            ...
            <table className="w-full">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-600 uppercase">Nama Jurusan</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-600 uppercase">Visi</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-600 uppercase">Misi</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-600 uppercase">Diperbarui</th>
              ...
              <tbody className="divide-y divide-gray-200">
                {filtered.map((item, idx) => (
                  <tr key={item.id_jurusan} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 text-sm font-medium text-gray-900">{item.nama_jurusan}</td>
                    <td className="px-6 py-4 text-sm text-gray-600 max-w-xs">
                      <p className="line-clamp-2">{item.visi ?? <span className="italic text-gray-400">—</span>}</p>
                    </td>
                    <td className="px-6 py-4 text-sm text-gray-600 max-w-xs">
                      <p className="line-clamp-2">{item.misi ?? <span className="italic text-gray-400">—</span>}</p>
                    </td>
        ...
        {!isLoading && filtered.length > 0 && (
          <div className="px-6 py-4 border-t border-gray-200">
            <p className="text-sm text-gray-600">
              Menampilkan <span className="font-medium">{filtered.length}</span> dari{" "}
              <span className="font-medium">{data.length}</span> jurusan
            </p>
          </div>
        )}
      </div>
          ...
          <div className="space-y-4">
            <div>
              ...
              <input
                ...
                onChange={(e) => setForm((f) => ({ ...f, nama_jurusan: e.target.value }))}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-lg shadow-2xl w-full max-w-md mx-4 p-6">
            <div className="flex items-start gap-4">
              ...
              <div>
                <h3 className="text-lg font-semibold text-gray-900">Hapus Jurusan</h3>
