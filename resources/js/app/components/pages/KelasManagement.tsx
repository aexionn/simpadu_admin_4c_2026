...
const kelasData = [
  { id: 1, kode: "TI-1A", nama: "Teknik Informatika 1A", prodi: "D3 Teknik Informatika", tahunAkademik: "2025/2026", semester: 1, dosen: "Dr. Ahmad Yani, M.Kom.", jumlahMhs: 35, status: "Aktif" },
  { id: 2, kode: "TI-1B", nama: "Teknik Informatika 1B", prodi: "D3 Teknik Informatika", tahunAkademik: "2025/2026", semester: 1, dosen: "Prof. Siti Nurhaliza, M.Kom.", jumlahMhs: 32, status: "Aktif" },
  { id: 3, kode: "TI-2A", nama: "Teknik Informatika 2A", prodi: "D3 Teknik Informatika", tahunAkademik: "2024/2025", semester: 3, dosen: "Dr. Bambang Sutrisno, M.T.", jumlahMhs: 30, status: "Aktif" },
  { id: 4, kode: "TI-3A", nama: "Teknik Informatika 3A", prodi: "D3 Teknik Informatika", tahunAkademik: "2023/2024", semester: 5, dosen: "Dr. Rina Wati, M.Kom.", jumlahMhs: 28, status: "Aktif" },
  { id: 5, kode: "SI-1A", nama: "Sistem Informasi 1A", prodi: "D3 Sistem Informasi", tahunAkademik: "2025/2026", semester: 1, dosen: "M. Rizal, M.Kom.", jumlahMhs: 33, status: "Aktif" },
  { id: 6, kode: "SI-2B", nama: "Sistem Informasi 2B", prodi: "D3 Sistem Informasi", tahunAkademik: "2024/2025", semester: 3, dosen: "Dr. Sri Lestari, M.Kom.", jumlahMhs: 29, status: "Aktif" },
];

function mapKelas(item: any) {
  const nama = valueOf(item.KELAS_NAMA, item.NAMA_KELAS, item.nama_kelas, item.nama, item.name);

  return {
    id: valueOf(item.ID_KELAS, item.ID_KELAS_MASTER, item.id_kelas, item.id, nama),
    kode: valueOf(item.KODE_KELAS, item.kode_kelas, item.kode, item.ALIAS, item.alias, nama),
    nama,
    ...
    dosen: valueOf(item.dosen?.name, item.DOSEN_WALI, item.dosen_wali, item.dosen, "-"),
    jumlahMhs: Number(valueOf(item.JUMLAH_MAHASISWA, item.jumlah_mahasiswa, item.jumlahMhs, 0)),
  ...
  return (
    <div className="p-8">
      ...
      <div className="mb-6">
        ...
        <p className="text-gray-600 mt-1">Kelola data kelas di POLIBAN</p>
      </div>
          ...
          <table className="w-full">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-600 uppercase">Kode Kelas</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-600 uppercase">Nama Kelas</th>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-600 uppercase">Dosen Wali</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-600 uppercase">Jumlah Mhs</th>
            ...
            <tbody className="divide-y divide-gray-200">
              {filteredData.map((item, index) => (
                <tr key={item.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 text-sm text-gray-600">{item.jumlahMhs}</td>
        ...
        <form onSubmit={handleSubmit} className="p-6">
          <div className="space-y-4">
            ...
            <div>
              ...
              <input
                type="number"
                defaultValue={editData?.jumlahMhs}
                placeholder="0"
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              />
            </div>
